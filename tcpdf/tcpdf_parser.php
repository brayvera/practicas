...
// -----------------------------------------------------------------------------
	...
	protected function getXrefData($offset=0, $xref=array()) {
		if ($offset == 0) {
			...
			if (preg_match_all('/[\r\n]startxref[\s]*[\r\n]+([0-9]+)[\s]*[\r\n]+%%EOF/i', $this->pdfdata, $matches, PREG_SET_ORDER, $offset) == 0) {
				$this->Error('No se puede encontrar startxref');
			}
		...
		if (empty($xref)) {
			$this->Error('No se puede encontrar la referencia externa');
		}
		return $xref;
	}
	...
	protected function decodeXref($startxref, $xref=array()) {
		...
		if (preg_match('/trailer[\s]*<<(.*)>>/isU', $this->pdfdata, $matches, PREG_OFFSET_CAPTURE, $offset) > 0) {
			...
			$this->Error('No se puede encontrar trailer');
		}
		return $xref;
	}
				...
				for ($i=1; $i<=$columns; ++$i) {
					...
					switch ($predictor) {
						...
						default: { // PNG prediction (on encoding, PNG optimum)
							$this->Error('Predictor PNG desconocido');
							break;
						}
					}
				}
	...
	protected function getIndirectObject($obj_ref, $offset=0, $decoding=true) {
		...
		if (($obj === false) OR (count($obj) != 2)) {
			$this->Error('Referencia de objeto no válida: '.$obj);
			return;
		}
